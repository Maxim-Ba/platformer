...
Заливка runtime-ассетов в MinIO с ПК требует SSH-туннеля, `kubectl port-forward` на сервере и клиента `mc` (не Midnight Commander). У разработчика нет рабочего kubeconfig на ноутбуке, поэтому каждый `assets:push` ломается. Нужен браузерный UI на отдельном хосте, который из кластера ходит в ClusterIP MinIO — без туннеля и без публичной MinIO Console.
...
- Документировать DNS A-запись, вход в UI, загрузку в префикс `assets/` (зеркало `public/assets/`). `npm run assets:push` / pre-push POST-ят файлы в s3manager по HTTPS с BasicAuth (креды `.env.local` или TTY prompt). `assets:pull` остаётся `mc` для Jenkins.
...
- `docs/DEPLOYMENT.md`, `docs/MINIO-ASSETS.md`, README: UI и `assets:push` (HTTP s3manager) как заливка с ПК; Jenkins `assets:pull` через `mc`
