...
Аутентифицированный веб-UI (s3manager) в namespace `platformer` для просмотра и загрузки объектов в bucket `platformer-assets` без kubectl на ноутбуке. UI доступен только на отдельном HTTPS-хосте с HTTP BasicAuth; секреты не хранятся в git.
