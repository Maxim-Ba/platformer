...
Переходы между комнатами внутри активной `GameScene`: доменная модель дверей, pure rules для spawn resolution, use case `TransitionThroughDoor`, hot-swap контента комнаты без restart сцены.
